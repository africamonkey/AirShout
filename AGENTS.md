...
| 文件 | 职责 |
|------|------|
| `P2PAudioManager.swift` | P2P 音频管理：MC 发现 + TCP 传输 |
| `TCPConnectionManager.swift` | TCP Server/Client 多连接管理 |
| `PeerMessage.swift` | PeerInfo 和地址交换消息 |
| `DeviceIdentifier.swift` | UUID 生成和持久化 |
| `PeerAddressStore.swift` | 对端 IP:Port 缓存 |
| `NetworkMonitor.swift` | IP 变化检测 |
...
- `docs/superpowers/plans/2026-04-16-p2p-tcp-refactoring.md` - P2P TCP 重构计划
...
#### MultipeerConnectivity 发现问题

**问题**：两台设备在连接前无法互相发现，因为：
- Receiver 只广告（不浏览）
- Sender 只浏览（不广告）
...
**问题**：在 MC 连接建立后才交换地址，而不是启动时。

**解决方案**：在 `MCSessionDelegate.didChange state = .connected` 时发送地址信息
```swift
case .connected:
...
**问题**：MC 连接成功不代表 TCP 连接成功，需要在收到对方地址后主动连接。

**解决方案**：在 `session(_:didReceive:fromPeer:)` 收到地址信息后建立 TCP 连接
```swift
case .addressInfo(let deviceID, ...):
    if self.currentRole() == .sender, self.pendingConnectionPeer?.deviceID == deviceID {
        Task {
            try await self.tcpManager?.connect(to: info)
        }
    }
```

#### pendingConnectionPeer 生命周期

**问题**：`pendingConnectionPeer` 设置后从不清理，导致状态混乱。

**解决方案**：在以下时机清理：
- MC 断开时
- TCP 连接成功时
- TCP 连接失败时
- cleanupCurrentRole() 时

#### UI 事件必须绑定处理函数

**问题**：SwiftUI View 中如果只定义了 UI 元素但没有绑定事件处理，用户操作不会有任何反应。

**修复**：确保所有可交互元素都绑定了正确的回调
```swift
...
所有可变状态都通过 `roleQueue.sync` 访问：
```swift
private var speakingEngineRunning = false
private let roleQueue = DispatchQueue(label: "com.airshout.p2prole")

private func isSpeakingEngineRunning() -> Bool {
    roleQueue.sync { speakingEngineRunning }
}

private func tryStartSpeaking() -> Bool {
    roleQueue.sync {
        guard !speakingEngineRunning else { return false }
...
#### 避免在音频线程中使用 DispatchQueue.sync

tap 回调在音频线程执行，`roleQueue.sync` 会阻塞音频线程导致音频断续。使用直接访问：
```swift
inputNode.installTap(onBus: 0, bufferSize: 1024, format: inputFormat) { [weak self] buffer, _ in
    guard let self = self, self.speakingEngineRunning else { return }  // 直接访问
    // ...
}
```
...
#### AudioPacketHeader 内存对齐

**问题**：结构体有 padding，直接用 `withUnsafeBytes` 会复制垃圾数据。

**解决方案**：手动序列化，不依赖结构体内存布局
```swift
